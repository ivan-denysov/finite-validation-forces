"""
ПАСЬЯНС-2d: индивидуальные пороги (квантиль СОБСТВЕННОГО фона узла) -> чистый Пуассон?
+ фиксация расщепления стабильные/нестабильные при общем пороге (v3) как наблюдения.
"""
import numpy as np
...
# индивидуальные фоны
per={h:[] for h in heavy0}
thb=th.copy()
for t in range(6000):
    thb=step(thb,om)
    if t%20==0:
        for h in heavy0: per[h].append(mis(thb,h))
q=0.995
dE={h:np.quantile(per[h],q) for h in heavy0}
print(f"индивидуальные пороги q={q}: min={min(dE.values()):.4f}, max={max(dE.values()):.4f}")
# распад
...
dec=len(times)
print(f"распалось {dec}/{M}, выжило {len(heavy)}")
if dec>=15:
    ...
    print(f"выживание: tau={tau:.0f}, R²={r2:.4f}")
